        ...
        if (file) {
            ...
            reader.onload = async (event) => {
                try {
                    ...
                    const productsRequiringTranslation = productsToImport.filter(p => !p.nameTamil || p.nameTamil.trim() === '');
                    ...
                    if (namesToTranslate.length > 0) {
                        ...
                        for (let i = 0; i < totalBatches; i++) {
                            ...
                            const batchNames = namesToTranslate.slice(start, end);
                            
                            setStatusMessage(`Translating batch ${i + 1} of ${totalBatches} (${batchNames.length} items)...`);
                            
                            try {
                                const translatedBatch = await translateBatchToTamilTransliteration(batchNames);
                                batchNames.forEach((name, index) => {
                                    if (translatedBatch[index]) {
                                        translationMap.set(name, translatedBatch[index]);
                                    }
                                });
                            } catch (batchError) {
                                console.error(`Batch translation failed for batch ${i+1}:`, batchError);
                    ...
                    const finalProducts = productsToImport.map(p => {
                        if ((!p.nameTamil || p.nameTamil.trim() === '') && translationMap.has(p.name)) {
